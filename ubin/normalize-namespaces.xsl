...
<xsl:transform xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  -->
  <xsl:variable name="almost-unique-uri-namespace-nodes"
    select="//namespace::*[name()!='xml']
    [not(.=../preceding::*/namespace::* or .=ancestor::*[position()&gt;1]/namespace::*)]"/>
        ...
        <prefix>
          <xsl:choose>
            ...
            <xsl:when test="not(name())
              and (//*[namespace-uri()=''] or //@*[namespace-uri()=current()])">
              <xsl:variable name="alternate-prefix-candidate"
                select="//namespace::*[count(.|current())!=1][.=current()][name()!=''][1]"/>
              <xsl:choose>
  ...
  <xsl:variable name="unique-uri-bindings"
    select="exsl:node-set($almost-unique-uri-bindings-tree)/binding[not(uri=preceding::uri)]"/>
  ...
  -->
  <xsl:variable name="created-namespace-nodes"
    select="exsl:node-set($created-namespace-nodes-tree)//namespace::*"/>
  ...
  <xsl:template match="*" name="copy">
    ...
    <xsl:variable name="prefix"
      select="name($created-namespace-nodes[.=namespace-uri(current())])"/>
    <xsl:variable name="maybe-colon">
      <xsl:if test="$prefix">:</xsl:if>
    </xsl:variable>
    <xsl:element name="{$prefix}{$maybe-colon}{local-name()}"
  ...
  <xsl:template match="@*">
    ...
    <xsl:attribute name="{$prefix}{$maybe-colon}{local-name()}"
      namespace="{namespace-uri()}">
      <xsl:value-of select="."/>
    </xsl:attribute>
  </xsl:template>
